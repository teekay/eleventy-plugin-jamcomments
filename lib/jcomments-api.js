...
/**
 * Reads comments for your website from your JComments instance and puts them in a JSON cache.
 *
 * @param {string} apiUrl - Full URL of your JComments instance, e.g.: https://jcomm-staging-eus-webapp.azurewebsites.net/api/comments
...
async function loadComments(apiUrl, apiToken, format, pathToCache, useCached) {
  if (!apiUrl || !apiToken) {
    throw new Error('JComments API URL & API token are required to fetch your comments');
  }
...
function commentsRendered(comments, dateFormat, noFollow) {
  ...
  return `<section class="jcomments comments">
